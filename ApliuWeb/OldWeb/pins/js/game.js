...
, c.save().font("bold 18px sans-serif").textAlign("right").fillStyle("#060").fillText("恭喜过关！", h - 10, g - 60).fillText("稍后一秒。", h - 10, g - 30).restore(), setTimeout(l, 1e3)))
            ...
            c.save().font("bold 5px sans-serif").textAlign("left").fillStyle("#000").fillText(" 时间:" + (time) + " 分数:" + F * F * F, 10, g - 60).font("bold 48px sans-serif").textAlign("center").fillStyle("#fff").fillText(F, m, d).restore(), N(r)
        }
    }
...
function share() {
    window.location.href = "../login/login.html?id=pins";
}
